...
module.exports.run = async (bot,message,args) => {
    ...
    await mess.react("✅");
    await mess.react("❎");
    const collector = mess.createReactionCollector((reaction, user) => reaction.emoji.name === '✅' || reaction.emoji.name === '❎' && user.id == message.author.id, {time: 30000});

    collector.on('collect', async r =>{
        switch(r.emoji.name){
            case '✅':
                await mess.edit(voting + '| Да');
                choise = 'Да';
            break;
            case '❎':
                await mess.edit(voting + '| Нет');
                choise = 'Нет';
            break;
        }
    })
